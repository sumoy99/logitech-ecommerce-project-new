...
class Updater extends Controller
{
    ...
    function update(Request $request){
		...
		$product_current_version = DB::table('global_settings')->where('key', 'version')->value('value');

        $rules = array('file' => 'required|file|mimes:zip');
        $validator = Validator::make($request->all(), $rules);
        if ($validator->fails()){
            return redirect()->back()->with('warning', 'Select a valid zip file');
        }
        ...
        if (class_exists('ZipArchive')) {
            ...
            return redirect()->back()->with('warning', 'Your server is unable to extract the zip file').'. '.('Please enable the zip extension to the server, then try again');
        }
        ...
        if(file_exists(base_path($dir.'/'.$uploaded_folder_name.'/step1_pre_checker.php'))){
		...
		if(array_key_exists('is_addon', $config) && strval($config['is_addon']) == "1"){
			...
			if(isset($request->purchase_code) && $request->purchase_code != ''){
				$response = (new AdminController)->curl_request($request->purchase_code);
				
				if($response != 1) {
					return redirect()->back()->with('error', 'Purchase code verification failed');
				} else{
					$duplicate_code_check = DB::table('addons')->where('purchase_code', $request->purchase_code)->get();
					if(count($duplicate_code_check) > 0){
						return redirect()->back()->with('error', 'You have already use this purchase code');
					}
				}
			...
			if($config['product_version']['minimum_required_version'] > $product_current_version){
				...
				return redirect()->back()->with('warning', "You have to update your main application's version.".'('.$config['product_version']['minimum_required_version'].') '.' to install the addon');
			}
			...
			if($addons_current_version->get()->count() > 0){
				...
				$addons_current_version = $config['addon_version']['minimum_required_version'];
			}
			...
			if(strval($config['addon_version']['minimum_required_version']) > strval($addons_current_version)){
				return redirect()->back()->with('warning', 'It looks like you are skipping a version'.'. '.'Please update version'.' '.$config['addon_version']['minimum_required_version'].' '.'first');
			}
			...
			foreach($config['addons'] as $addon){
				...
				$addons_details = DB::table('addons')->where('unique_identifier', $config['addons'][0]['unique_identifier']);

				if($addons_details->get()->count() > 0){
					DB::table('addons')->where('id', $addons_details->value('id'))->update($data);
					$parent_id = $addons_details->value('id');
				} else {
			...
			if(strval($config['product_version']['minimum_required_version']) != strval($product_current_version)){
				return redirect()->back()->with('warning', 'It looks like you are skipping a version'.'. '.'Please update version'.' '.$config['product_version']['minimum_required_version'].' '.'first');
			}
		}
		...
		//run SQL file
		if(file_exists(base_path($dir.'/'.$uploaded_folder_name.'/step3_database.sql'))){
        ...
        // Exicute php or laravel's code
        if(file_exists(base_path($dir.'/'.$uploaded_folder_name.'/step4_update_data.php'))){
        ...
        if(array_key_exists('is_addon', $config) && strval($config['is_addon']) == "1"){
        	if($config['addon_version']['minimum_required_version'] == "0"){
        		...
        		return redirect()->back()->with('message', 'Addon installed successfully');
        	}else{

        		return redirect()->back()->with('message', 'Addon updated successfully');
        	}
        }else{
        	DB::table('global_settings')->where('key', 'version')->update(['value' => $config['product_version']['update_version']]);
