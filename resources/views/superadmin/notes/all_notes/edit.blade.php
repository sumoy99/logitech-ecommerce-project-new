<form action="{{ route('superadmin.notes.all_notes.update', ['id' => $note->id]) }}" method="POST" enctype="multipart/form-data">
    @csrf
    ...
    <div class="form-group mb-3">
        ...
        <input type="text" id="title" name="title" value="{{$note->title}}" class="form-control" placeholder="Note title" required>
        <small id="titleCharCount" class="text-muted">0 / 200 characters</small>
    </div>

    <div class="form-group">
        <label for="description">{{ get_phrase('Description') }}*</label>
        <textarea class="form-control" id="description" name="description" rows="5" style="height: 156px;">{{$note->description}}</textarea>
    ...
    <div class="mb-3">
        ...
        <select class="form-select form-select-lg" id="visibility" name="visibility">
            <option value="all" {{ $note->visibility == 'all' ? 'selected' : '' }}>All Users</option>
            <option value="customer" {{ $note->visibility == 'customer' ? 'selected' : '' }}>Customer</option>
            <option value="vendor" {{ $note->visibility == 'vendor' ? 'selected' : '' }}>Vendor</option>
            <option value="marcent" {{ $note->visibility == 'marcent' ? 'selected' : '' }}>Marcent</option>
        </select>
    </div>


    <div class="form-check form-switch mb-3">
        <input class="form-check-input" type="checkbox" name="status" id="status"  {{ $note->status ? 'checked' : '' }}>
        <label class="form-check-label" for="status">Active</label>
    </div>

    <button type="submit" class="btn btn-primary w-100">{{ get_phrase('Update Note') }}</button>
</form>
