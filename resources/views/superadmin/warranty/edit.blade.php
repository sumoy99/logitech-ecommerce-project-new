<form action="{{ route('superadmin.warranty.update', ['id' => $warranty->id]) }}) }}" method="POST" enctype="multipart/form-data">
    @csrf
    <div class="form-group mb-3">
        <label for="title" class="form-label">{{ get_phrase('Title') }}</label>
        <input type="text" id="title" name="title" class="form-control" value="{{$warranty->title}}" placeholder="Warranty title" required>
    ...
    <div class="form-group">
        ...
        {{-- Live preview area --}}
        <div class="mt-3">
            <img id="logoPreview" src="{{ $warranty->logo ? asset('assets/upload/warranty/' . $warranty->logo) : asset('assets/backend/assets/img/placeholder.png') }}" alt="Current Logo" height="80" style="border: 1px solid #ddd; padding: 5px;">
        </div>
    </div>
