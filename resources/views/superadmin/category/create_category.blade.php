<form action="{{ route('superadmin.category.category_store') }}" method="POST" enctype="multipart/form-data" class="">
  @csrf

  {{-- <h4 class="mb-4 text-primary fw-semibold">Add New Category</h4> --}}

  <!-- Category Name -->
  <div class="mb-3">
    <label for="name" class="form-label">{{ get_phrase('Category Name') }}</label>
    <input type="text" class="form-control form-control-lg" id="name" name="name" placeholder="Type category name" required>
  </div>

  <!-- Parent Category -->
  <div class="mb-3">
    <label for="parent_id" class="form-label">{{ get_phrase('Parent Category') }} <small class="text-muted">({{ get_phrase('optional') }})</small></label>
  ...
  <button type="submit" class="btn btn-primary w-100 py-2">
    <i class="bi bi-plus-circle me-1"></i> {{ get_phrase('Add Category') }}
  </button>
</form>
