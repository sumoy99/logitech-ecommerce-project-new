<form action="{{ route('superadmin.category.category_update',  ['id' => $category->id]) }}" method="POST" enctype="multipart/form-data" class="">
    @csrf    

    <!-- Category Name -->
    <div class="mb-3">
        <label for="name" class="form-label">{{ get_phrase('Subcategory Name') }}</label>
        <input type="text" value="{{$category->name}}" class="form-control form-control-lg" id="name" name="name" placeholder="Type category name" required>
    </div>
    ...
    <div class="form-group mb-3">
        ...
        <input type="text" id="meta_title" name="meta_title" value="{{$category->meta_title}}" class="form-control" placeholder="Meta title" required>
        <small id="metatitleCharCount" class="text-muted">0 / 100 characters</small>
    </div>

    <div class="form-group mb-3">
        <label for="meta_desc">{{ get_phrase('Meta Description') }}</label>
        <textarea class="form-control" id="meta_desc" name="meta_desc" rows="5" style="height: 156px;">{{$category->meta_desc}}</textarea>
        <small id="descCharCount" class="text-muted">0 / 200 characters</small>
    </div>

    <div class="form-group mb-3">
        <label for="seo_desc">{{ get_phrase('SEO Description') }}</label>
        <textarea class="form-control" id="seo_desc" name="seo_desc" rows="5" style="height: 156px;">{{$category->seo_desc}}</textarea>
    ...
    <button type="submit" class="btn btn-primary w-100 py-2">
      <i class="bi bi-plus-circle me-1"></i> {{ get_phrase('Update Category') }}
    </button>
  </form>
