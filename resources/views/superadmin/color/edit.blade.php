<style>
  ...
  <form action="{{ route('superadmin.color.update', ['id' => $color->id]) }}" method="POST">
    @csrf
    <div class="mb-3">
      <label for="name" class="form-label">Color Name</label>
      <input type="text" name="name" value="{{$color->name}}" class="form-control" placeholder="e.g., Red" required>
    </div>
  
    <div class="mb-3">
      <label for="hex_code" class="form-label">Color Code</label>
      <div class="d-flex align-items-center gap-3">
        <input type="color" name="hex_code" id="hex_code" class="form-control form-control-color" value="{{$color->hex_code}}" required>
        <span id="color_value">{{$color->hex_code}}</span>
      </div>
      <div class="form-text">Pick a color to see the code</div>
    </div>
  
    <div class="form-check form-switch mb-3">
        <input class="form-check-input" type="checkbox" name="status" id="status" {{ $color->status == 'on' ? 'checked' : '' }}>
        <label class="form-check-label" for="status">Active</label>
      </div>
  
    <button type="submit" class="btn btn-primary w-100">Update Color</button>
  </form>
