@extends('frontend.navigation')
   <title>Shopping Cart | Logitech</title>
@section('content')
...
<!-- page-cart -->
    ...
    @include('partials.page-title', ['title' => 'Shopping Cart', 'subtitle' => 'Shop through our latest selection of Fashion'])
            ...
            <div class="container">
                ...
                <div class="tf-cart-countdown">
                    <div class="title-left">
                        ...
                        <p>Everything looks good! Checkout now and get fast delivery.</p>
                    </div>
                </div>
                <form action="{{route('frontend.customer.check_out')}}" method="post">
                    @csrf
                                ...
                                <tbody>
                                    @foreach ($cartItems as $item)
                                        @if ($item->stock_status == 'In stock')
                                            ...
                                            <tr class="tf-cart-item file-delete" data-price="{{ $item->final_price }}">
                                                <td class="tf-cart-item_product">
                                                    <a href="{{ route('frontend.product_details', ['product_slug' => $item->slug, 'id' => $item->id]) }}" class="img-box">
                                                        <img src="{{ $item->thumbnail ? asset('assets/upload/products/thumbnails/' . $item->thumbnail) : asset('assets/backend/assets/img/placeholder.png') }}" alt="img-product">
                                                    </a>
                                                    <div class="cart-info">
                                                        <a href="{{ route('frontend.product_details', ['product_slug' => $item->slug, 'id' => $item->id]) }}" class="cart-title link">{{$item->name}}</a>

                                                        @foreach ($item->options as $key => $value)
                                                            <div class="cart-meta-variant">{{ ucfirst($key) }}: {{ $value }}</div>
                                                        @endforeach
                                                        
                                                        <span class="remove-cart link "><a href="{{route('frontend.customer.CartPrdRmv', ['id' => $item->id])}}">Remove</a></span>
                                                    </div>  
                                                </td>
                                                <td class="tf-cart-item_price tf-variant-item-price" cart-data-title="Price">
                                                    <div class="cart-price price">{{ currency($item->final_price) }}</div>
                                                </td>
                                                <td class="tf-cart-item_quantity" cart-data-title="Quantity">
                                                    <div class="cart-quantity">
                                                        <div class="wg-quantity">
                                                            <span class="btn-quantity btndecrease2">
                                                            ...
                                                            <span class="btn-quantity btnincrease2">
                                                ...
                                                <td class="tf-cart-item_total tf-variant-item-total" cart-data-title="Total">
                                                    <div class="cart-total ">{{ currency($totalPrice) }}</div>
                                                </td>
                                            </tr>
                                        @endif
                                    @endforeach
                        ...
                        <div class="tf-page-cart-footer">
                            <div class="tf-cart-footer-inner">
                                ...
                                <div class="tf-page-cart-checkout">
                                    <div class="tf-cart-totals-discounts"  cart-data-title="Total">
                                        <h3>Subtotal</h3>
                                        <span class="total-value price">{{currency($totalPrice)}}</span>
                                    </div>
                                    <p class="tf-cart-tax">
                                    ...
                                    <div class="cart-checkout-btn">
                                        {{-- <a href="{{route('frontend.customer.check_out')}}"
                                            class="tf-btn w-100 btn-fill animate-hover-btn radius-3 justify-content-center">
                                            <span>Check out</span>
                                        </a> --}}
                                        <button type="submit" class="tf-btn w-100 btn-fill animate-hover-btn radius-3 justify-content-center"><span>Check out</span></button>
    ...
    @else
        <div class="tf-page-cart text-center mt_140 mb_200">
            ...
            <p class="mb_24">You may check out all the available products and buy some in the Home</p>
            <a href="{{route('frontend.index')}}" class="tf-btn btn-sm radius-3 btn-fill btn-icon animate-hover-btn">Return to shop<i class="icon icon-arrow1-top-left"></i></a>
        </div> 
    @endif
        <!-- page-cart -->
@endsection
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const subtotalElement = document.querySelector('.total-value');
        ...
        function calculateSubtotal() {
            ...
            document.querySelectorAll('.tf-cart-item').forEach(row => {
                ...
                row.querySelector('.cart-total').textContent = '৳' + total.toFixed(2);
                subtotal += total;
            });
        ...
        // --- Handle events ---
        document.querySelectorAll('.tf-cart-item').forEach(row => {
            const decreaseBtn = row.querySelector('.btndecrease2');
            const increaseBtn = row.querySelector('.btnincrease2');
            const qtyInput = row.querySelector('input[name="itemQuantity[]"]');

            // Prevent double event firing by using event delegation properly
            ...
            // Manual typing in input
            qtyInput.addEventListener('input', function() {
                updateQuantity(row, this.value);
            });

            // Blur event ensures manual input gets validated
